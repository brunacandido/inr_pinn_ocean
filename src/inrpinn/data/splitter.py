...
class GlorysProfileSplitter:
    ...
    def split(
        ...
        data_fraction : float | None
            If set (e.g. 0.1 for 10 %), randomly keep only that fraction of
            all profiles before splitting.  Applied after ``weekly_subsample``
            if both are set.  Controlled by ``seed``.
        ...
        if data_fraction is not None:
            ...
            n_keep      = max(1, round(data_fraction * len(unique_pids)))
            pids_copy   = unique_pids.copy()
            rng.shuffle(pids_copy)
            unique_pids = np.sort(pids_copy[:n_keep])
        ...
        if mode == "uniform":
            ...
            # Step 2: training from non-val pool
            ...
            val_pids, all_square_pids = self._sample_val_contiguous(
                rng, val_fraction, train_fraction, n_val_squares, unique_pids,
            )

            # Test = square profiles not assigned to val
            test_pids = all_square_pids[~np.isin(all_square_pids, val_pids)]
    ...
    # Places n_squares non-touching spatial boxes sized to cover val+test fraction.
    # Returns (val_pids, all_square_pids): val is a random subset of square profiles,
    # test is the remainder (caller assigns test = all_square_pids \ val_pids).
    def _sample_val_contiguous(
        ...
        # Size boxes to cover val + test = (1 - train_fraction) of the domain.
        held_out_fraction = 1.0 - train_fraction
        target_area = held_out_fraction / n_squares * lat_range * lon_range
        half_side   = np.sqrt(target_area) / 2.0

        half_lat = min(half_side, lat_range / 2.0 * 0.95)
        half_lon = min(half_side, lon_range / 2.0 * 0.95)

        # Minimum gap between box edges: 2 grid cells in each direction.
        ...
        placed: list[tuple[float, float]] = []   # (c_lat, c_lon) of accepted boxes
        all_pid_set: set[int] = set()
        ...
        for i in range(n_squares):
            ...
            for _ in range(500):
                ...
                # A new box is acceptable if it is separated from every placed box
                # by at least min_gap in lat OR in lon (axis-aligned non-touching).
                ok = all(
                    abs(c_lat - p_lat) - 2 * half_lat >= min_gap_lat
                    or abs(c_lon - p_lon) - 2 * half_lon >= min_gap_lon
                    for p_lat, p_lon in placed
                )
                if ok:
                    accepted = True
                    break
            ...
            if not accepted:
                ...
                _w.warn(
                    f"Could not place non-touching square {i+1}/{n_squares} after 500 "
                    "attempts — placing at best available position.",
                    stacklevel=3,
                )

            placed.append((c_lat, c_lon))
            in_box = (
                (pid_lats >= c_lat - half_lat) & (pid_lats <= c_lat + half_lat)
                & (pid_lons >= c_lon - half_lon) & (pid_lons <= c_lon + half_lon)
            )
            all_pid_set.update(unique_pids[in_box].tolist())

        all_square_pids = np.array(sorted(all_pid_set), dtype=unique_pids.dtype)

        # Split square profiles into val and test using val_fraction of total.
        n_val      = min(
            max(1, round(val_fraction * len(unique_pids))),
            len(all_square_pids),
        )
        shuffled   = all_square_pids.copy()
